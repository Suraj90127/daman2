...
const validateBet = async (join, list_join, x, money, game) => {
    ...
    if (checkJoin) {
        let arr = String(list_join)?.split('');
...
const listOrderOld = async (req, res) => {
    ...
    const [k5d] = await connection.query(`SELECT * FROM 5d WHERE status != 0 AND game = '${game}' ORDER BY id DESC LIMIT ${limit} OFFSET ${offset}`);
...
const GetMyEmerdList = async (req, res) => {
    ...
    const [result_5d] = await connection.query(`SELECT * FROM result_5d WHERE phone = ? AND game = '${game}' ORDER BY id DESC  LIMIT ${limit} OFFSET ${offset}`, [user[0].phone]);
...
const add5D = async(game) => {
    try {
        ...
        if (game == 1) updatenum = 5;
        if (game == 3) updatenum = 6;
        if (game == 5) updatenum = 7;
        if (game == 10) updatenum = 8;
        ...
        if (k5D[0] === undefined) {
            ...
            period = `${times}0${updatenum}0000`;
            
        } else {
            // Get the period from the database
            let dbPeriod = k5D[0].period;
        
            // Extract the date part from the database period
            let ddd=202407310101
            let dbDatePart = dbPeriod.toString().slice(0, 8);
